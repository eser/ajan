...
func TestCorsMiddleware(t *testing.T) { //nolint:funlen
	...
	t.Run("default_options", func(t *testing.T) {
		...
		assert.Equal(t, "*", w.Header().Get(middlewares.AccessControlAllowOriginHeader))
		assert.Empty(t, w.Header().Get(middlewares.AccessControlAllowCredentialsHeader))
		assert.Empty(t, w.Header().Get(middlewares.AccessControlAllowHeadersHeader))
		assert.Empty(t, w.Header().Get(middlewares.AccessControlAllowMethodsHeader))
	})
